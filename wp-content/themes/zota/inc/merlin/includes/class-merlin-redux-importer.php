...
class Merlin_Redux_Importer {
	...
	public static function import( $import_data ) {
		...
		if ( ! class_exists( 'ReduxFramework' ) || ! class_exists( 'Redux_Instances' ) || empty( $import_data ) ) {
			return false;
		}
		...
		foreach ( $import_data as $redux_item ) {
			...
			$redux_framework        = Redux_Instances::get_instance( $redux_item['option_name'] );

			if ( isset( $redux_framework->args['opt_name'] ) ) {
				$redux_framework->options_class->set( $redux_options_data );

				Merlin_Logger::get_instance()->debug( __( 'The Redux Framework data was imported' , 'zota'), $redux_item );
			}
		}
