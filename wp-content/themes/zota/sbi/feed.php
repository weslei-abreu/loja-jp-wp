...
if( !empty($zota_atts['tb-atts']) ) {
	...
	if( !empty($zota_atts['data-xlgdesktop']) ) {
		$cols_setting 				= $zota_atts['data-xlgdesktop'];
	} else {
		$cols_setting 				= $zota_atts['data-items'];
	}
	...
	<div id="sb_instagram" class="sbi sbi_col_<?php echo esc_attr( $cols_setting ); ?>" data-feedid="<?php echo esc_attr( $feed_id ); ?>" data-res="<?php echo esc_attr( $image_resolution_setting ); ?>" data-num="<?php echo esc_attr( $num_setting ); ?>">
		...
		<div id="sbi_images" <?php zota_instagram_feed_row_class($zota_atts); ?>>
			<?php
