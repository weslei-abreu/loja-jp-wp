...
class Tbay_Import {
	...
	public function __construct() {
 	...
 	public function create_admin_menu() {
		...
		$theme_name = ucfirst($theme_info->get( 'TextDomain' ));

		add_menu_page(
			__( $theme_name. ' Import', 'tbay-elementor' ),
			__( $theme_name. ' Import', 'tbay-elementor' ),
			'manage_options',
			'tbay-import-demo',
	...
	public function get_remote_sampledata() {
 		if ( isset($_GET['doaction']) && $_GET['doaction'] == 'download-sample' ) {
			...
			if ( $theme_name ) {
				...
				$remote_file = 'https://bitbucket.org/devthembay/update-plugin/raw/master/demosamples/'.$theme_name.'.zip';
	...
	public function import_redux_option($source) {
		...
		$file = TBAY_IMPORT_SAMPLES_DIR.'data/'.$source.'/redux_options.json';
		if ( file_exists($file) ) {
	...
	public function import_first_settings($source) {
		...
		if ( file_exists($file) ) {
			...
			if ( count( array_filter( $datas ) ) < 1 ) {
				return $this->outputJson( false, esc_html__( 'Data is error! file: ', 'tbay-elementor') . $file, '' );
			}
	...
	public function import_first_settings2($source) {
		...
		if ( file_exists($file) ) {
			...
			if ( count( array_filter( $datas ) ) < 1 ) {
				return $this->outputJson2( false, esc_html__( 'Data is error! file: ', 'tbay-elementor') . $file, '' );
			}
	...
	public function import_content($source) {
		...
		$source_parent = $sources[0]; 

		$path = TBAY_IMPORT_SAMPLES_DIR.'data/'.$source_parent.'/'.$file_name;
		if ( file_exists($path) ) {
			...
			require_once TBAY_ELEMENTOR_DIR . 'importer/wordpress-importer.php';
	...
	public function import_revslider($source) {
		...
		if ( is_dir(TBAY_IMPORT_SAMPLES_DIR . 'data/revslider/') ) {
			...
			$sources = explode( '/', $source );
			$source_parent = $sources[0];
			$path = TBAY_IMPORT_SAMPLES_DIR . 'data/' . $source_parent . '/revslider/';
		}
		...
		if ( is_dir($path) ) {
			...
			return $this->outputJson( false, esc_html__( 'revslider folder is not exists! folder: ', 'tbay-elementor') . $path, '' );
		}
	...
	public function import_settings($source) {
		...
		if ( file_exists($file) ) {
			...
			return $this->outputJson( false, esc_html__( 'File is not exists! file:', 'tbay-elementor') . $file, '' );
		}
	...
	public function import_settings2($source) {
		...
		if ( file_exists($file) ) {
			...
			return $this->outputJson2( false, esc_html__( 'File is not exists! file:', 'tbay-elementor') . $file, '' );
		}
	...
	public function tbay_page_content() {
		// script
		wp_enqueue_style( 'tbay-framework-backend', TBAY_ELEMENTOR_URL . 'assets/backend.css', array(), TBAY_ELEMENTOR_VERSION );
		wp_enqueue_script( 'tbay-framework-import', TBAY_ELEMENTOR_URL . 'assets/import.js', array( 'jquery' ), TBAY_ELEMENTOR_VERSION, true );
		...
		<div class="wrap">
			<h1><?php esc_html_e( 'TbayTheme Demo Importer', 'tbay-elementor' ); ?></h1>
			<div class="update-nag tbay_notification">
				<p>
					<?php _e( '<strong>Warning:</strong> If you have already used this feature before and you want to try it again, your content may be duplicated. Please consider resetting your database back to defaults with <a href="//wordpress.org/plugins/wordpress-reset/">this plugin</a>.', 'tbay-elementor' ); ?>
				</p>
			</div>
			...
			<div class="viewWrapper" id ="viewWrapper">
				...
				<div class="tbay-fw-dash-widget" id="system_dw">
					<div class="tbay-fw-dash-title-wrap <?php echo trim($class_status); ?>">
						<div class="tbay-fw-dash-title"><?php esc_html_e('System Requirements', 'tbay-elementor'); ?></div>
						<div class="tbay-fw-dash-title-button tbay-fw-status-red"><i class="icon-problem-found"></i><?php esc_html_e('Problem Found', 'tbay-elementor'); ?></div>
						<a class="tbay-fw-status-red tbay-fw-dash-title-button requirement-link" target="_blank" href="https://thembay.com"><i class="eg-icon-info"></i></a> <div class="tbay-fw-dash-title-button tbay-fw-status-green"><i class="icon-no-problem-found"></i><?php esc_html_e('No Problems', 'tbay-elementor'); ?></div>
					</div>
					<div class="tbay-fw-dash-widget-inner">

						<span class="tbay-fw-dash-label"><?php esc_html_e('Active parent theme', 'tbay-elementor'); ?></span>
						...
						<?php
							...
							if($is_parent_theme){
								...
								echo '<i class="revredicon eg-icon-cancel"></i><span style="margin-left:16px" class="rs-dash-more-info"><i class="eg-icon-info"></i><span class="rs-dash-red-content">'. esc_html__('You activated child theme, so you can not import the demo', 'tbay-elementor')  .'</span></span>';
							}
						?>
						...
						<span class="tbay-fw-dash-label"><?php esc_html_e('Uploads folder writable', 'tbay-elementor'); ?></span>
						...
						<?php
							...
							if($writeable_boolean){
								...
								echo '<i class="revredicon eg-icon-cancel"></i><span style="margin-left:16px" class="rs-dash-more-info" data-title="'.esc_html__('Error with File Permissions', 'tbay-elementor').'" data-content="'.esc_html__('Please set write permission (755) to your wp-content/uploads folders to make sure the Slider can save all updates and imports in the future.', 'tbay-elementor').'"><i class="eg-icon-info"></i></span>';
							}
						?>
						...
						<!-- Check php version -->
						<span class="tbay-fw-dash-label"><?php esc_html_e('PHP version', 'tbay-elementor'); ?></span>
						...
						<?php if( $phpversion < TBAY_RECOMMEND_PHP_VERSION ) : ?>
							...
							<span class="rs-dash-red-content"><?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $phpversion_str ); ?></span>
							<span class="rs-dash-strong-content" style="margin-left:20px"><?php printf( __( '<strong>(min: %1$s)</strong>', 'tbay-elementor' ), TBAY_RECOMMEND_PHP_VERSION ); ?></span>

						<?php else : ?>


							<span class="tbay-fw-dash-strong-content">
								<?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $phpversion_str ); ?>
							</span>	
						...
						<!-- Check Memory Limit -->
						<span class="tbay-fw-dash-label"><?php esc_html_e('Memory Limit', 'tbay-elementor'); ?></span>
						...
						<?php if( intval( $memory_limit_byte ) < TBAY_RECOMMEND_MEMORY_LIMIT ) : ?>
							...
							<span class="<?php echo ($is_memory_limit) ? 'rs-dash-strong-content' : 'rs-dash-red-content'; ?>"><?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $memory_limit ); ?></span>
							<span class="rs-dash-strong-content" style="margin-left:20px"><?php _e('<strong>(min:256M)</strong>', 'tbay-elementor'); ?></span>

						<?php else : ?>


							<span class="tbay-fw-dash-strong-content">
								<?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $memory_limit ); ?>
							</span>	
						...
						<!-- Upload Max Filesize -->
						<span class="tbay-fw-dash-label"><?php esc_html_e('Upload Max. Filesize', 'tbay-elementor'); ?></span>
						...
						<?php if( intval( $upload_max_filesize_byte ) < TBAY_RECOMMEND_UPLOAD_MAX_FILESIZE ) : ?>
							...
							<span class="<?php echo ($is_upload_max_filesize) ? 'rs-dash-strong-content' : 'rs-dash-red-content'; ?>"><?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $upload_max_filesize ); ?></span>
							<span class="rs-dash-strong-content" style="margin-left:20px"><?php _e('<strong>(min:32M)</strong>', 'tbay-elementor'); ?></span>

						<?php else : ?>


							<span class="tbay-fw-dash-strong-content">
								<?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $upload_max_filesize ); ?>
							</span>	
						...
						<!-- Max Post Size -->
						<span class="tbay-fw-dash-label"><?php esc_html_e('Max. Post Size', 'tbay-elementor'); ?></span>
						...
						<?php if( intval( $post_max_size_byte ) < TBAY_RECOMMEND_POST_MAX_SIZE ) : ?>
							...
							<span class="<?php echo ($is_post_max_size) ? 'rs-dash-strong-content' : 'rs-dash-red-content'; ?>"><?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $post_max_size ); ?></span>
							<span class="rs-dash-strong-content" style="margin-left:20px"><?php _e('<strong>(min:32M)</strong>', 'tbay-elementor'); ?></span>
						...
						<?php else : ?>
							...
							<span class="tbay-fw-dash-strong-content">
								<?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $post_max_size ); ?>
							</span>	
						...
						<span class="tbay-fw-dash-label"><?php esc_html_e('Max execution time', 'tbay-elementor'); ?></span>
						...
						<?php if( intval( $max_execution_time ) < TBAY_RECOMMEND_EXECUTION_TIME ) : ?>
							...
							<span class="<?php echo ($is_max_execution_time) ? 'rs-dash-strong-content' : 'rs-dash-red-content'; ?>"><?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $max_execution_time ); ?></span>
							<span class="rs-dash-strong-content" style="margin-left:20px"><?php printf( __( '<strong>(min: %1$s)</strong>', 'tbay-elementor' ), TBAY_RECOMMEND_EXECUTION_TIME ); ?></span>
						...
						<?php else : ?>
							...
							<span class="tbay-fw-dash-strong-content">
								<?php printf( __( 'Currently: %1$s', 'tbay-elementor' ), $max_execution_time ); ?>
							</span>	
				...
				<div class="tbay-fw-dash-widget" id="support_dw">
					<div class="tbay-fw-dash-title-wrap">
						<div class="tbay-fw-dash-title"><?php esc_html_e('Thembay Support', 'tbay-elementor'); ?></div>				
					</div>			
					<div class="tbay-fw-dash-widget-inner">			

						<div class="tbay-fw-dash-icon tbay-fw-dash-ticket"></div>
						<div class="tbay-fw-dash-content-with-icon">
							<div class="tbay-fw-dash-strong-content"><?php esc_html_e('Ticket support', 'tbay-elementor'); ?></div>
							<div><?php printf( __( 'Please send the ticker <a href="%1$s" target="_blank">here</a>', 'tbay-elementor' ), 'https://tickets.thembay.com/' ); ?></div>
						</div>
						<div class="tbay-fw-dash-content-space"></div>				
						<div class="tbay-fw-dash-icon tbay-fw-dash-mail"></div>
						<div class="tbay-fw-dash-content-with-icon">
							<div class="tbay-fw-dash-strong-content"><?php esc_html_e('Email Support', 'tbay-elementor'); ?></div>
							<div><?php printf( __( 'Please send the email: <a href="%1$s" target="_blank">here</a>', 'tbay-elementor' ), 'mailto:support@thembay.com' ); ?></div>
						</div>						

						<div class="tbay-fw-dash-content-space"></div>				
						<div class="tbay-fw-dash-icon tbay-fw-dash-youtube"></div>
						<div class="tbay-fw-dash-content-with-icon">
							<div class="tbay-fw-dash-strong-content"><?php esc_html_e('Youtube channel', 'tbay-elementor'); ?></div>
							<div><?php printf( __( 'Watch video tutorial <a href="%1$s" target="_blank">here</a>', 'tbay-elementor' ), 'https://www.youtube.com/channel/UCIkuoXjv4tS6SUHhEBAg9Ew' ); ?></div>
						</div>
					</div>
									...
									<div class="row container">
										<?php foreach ($value_theme as $key => $value) { ?>
											...
											<div class="tbay-demo-item-img">
												<img src="<?php echo (file_exists($path_dir . esc_attr($key_theme) . '/' . esc_attr($key) . '/' . $value['screenshot']) ? $path_uri . esc_attr($key_theme) . '/' . esc_attr($key) . '/' . $value['screenshot'] : TBAY_ELEMENTOR_URL . 'assets/none_image.jpg'); ?>" />
											</div>
											<div class="tbay-demo-item-action">
												<h3 style="display: none;"><?php echo esc_html($key_theme); ?></h3>
												<h2 style=""><?php echo $value['title']; ?></h2>
												<button class="button button-primary tbay-btn-import" <?php echo $attr_button_import; ?>><?php esc_html_e( 'Import All', 'tbay-elementor' ); ?>
									             <input class="hidden tbay-import-value" value="<?php echo esc_attr($key_theme) . '/' . esc_attr($key); ?>"/>
									            </button>												
									            <button class="button button-primary tbay-btn-config" <?php echo $attr_button_import; ?>><?php esc_html_e( 'Only Active', 'tbay-elementor' ); ?>
									             <input class="hidden tbay-import-value" value="<?php echo esc_attr($key_theme) . '/' . esc_attr($key); ?>"/>
									            </button>
											</div>
										</div>
						...
						<div class="update-nag-wrapper">
							<div class="content">
								...
								<div class="download-btn" style="text-align: left;">
									<?php
										$btn_html = '<a class="button button-primary" href="'.admin_url( 'tools.php?page=tbay-import-demo', 'http' ).'&doaction=download-sample">'.esc_html__('Download Demos', 'tbay-elementor').'</a>';
			...
			<div class="container">
				<div class="wrapper-content">
					...
					<button type="button" class="notice-dismiss tbay-close-import-popup">
						<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'tbay-elementor' ); ?></span>
					</button>
					<h1><?php esc_html_e( 'Importing', 'tbay-elementor' ); ?></h1>

					<div class="row">
						<div class="tbay_progress_import tbay_progress_import_1" style="display: none;">
							<p class="note"><?php esc_html_e( 'The import process can take about 10 minutes. Please don\'t refresh the page.', 'tbay-elementor' ); ?></p>
							<ol class="steps">
							<?php
								$steps = array(
									'first_settings' => array( 'default' => __('Install First Settings', 'tbay-elementor'), 'installing' => __('Installing First Settings ...', 'tbay-elementor'), 'installed' => __('Installed First Settings', 'tbay-elementor') ),
									'redux_option' => array( 'default' => __('Install Redux Option', 'tbay-elementor'), 'installing' => __('Installing Redux Option ...', 'tbay-elementor'), 'installed' => __('Installed Redux Option', 'tbay-elementor') ),
									'content' => array( 'default' => __('Install Demo Content', 'tbay-elementor'), 'installing' => __('Installing Demo Content ...', 'tbay-elementor'), 'installed' => __('Installed Demo Content', 'tbay-elementor') ),
									'widgets' => array( 'default' => __('Install Widgets', 'tbay-elementor'), 'installing' => __('Installing Widgets ...', 'tbay-elementor'), 'installed' => __('Installed Widgets', 'tbay-elementor') ),
									'settings' => array( 'default' => __('Install Settings', 'tbay-elementor'), 'installing' => __('Installing Settings ...', 'tbay-elementor'), 'installed' => __('Installed Settings', 'tbay-elementor') ),
									'revslider' => array( 'default' => __('Install Revolution Slider', 'tbay-elementor'), 'installing' => __('Installing Revolution Slider ...', 'tbay-elementor'), 'installed' => __('Installed Revolution Slider', 'tbay-elementor') ),
								);
						...
						<div class="tbay_progress_import tbay_progress_import_2" style="display: none;">
							...
							<?php
								$steps2 = array(
									'first_settings2 active' => array( 'default' => __('Install First Settings', 'tbay-elementor'), 'installing' => __('Installing First Settings ...', 'tbay-elementor'), 'installed' => __('Installed First Settings', 'tbay-elementor') ),
									'redux_option2' => array( 'default' => __('Install Redux Option', 'tbay-elementor'), 'installing' => __('Installing Redux Option ...', 'tbay-elementor'), 'installed' => __('Installed Redux Option', 'tbay-elementor') ),
									'widgets2' => array( 'default' => __('Install Widgets', 'tbay-elementor'), 'installing' => __('Installing Widgets ...', 'tbay-elementor'), 'installed' => __('Installed Widgets', 'tbay-elementor') ),
									'settings2' => array( 'default' => __('Install Settings', 'tbay-elementor'), 'installing' => __('Installing Settings ...', 'tbay-elementor'), 'installed' => __('Installed Settings', 'tbay-elementor') ),
								);
						...
						<div class="tbay_progress_error_message">
							<div class="tbay-error">
								<h4><?php esc_html_e( 'Failed to import!', 'tbay-elementor' ); ?></h4>
								<div class="content text_note tbay_notification"></div>
							</div>
							<div class="log update-nag tbay_notification">
								<h4><?php esc_html_e( 'Log', 'tbay-elementor' ); ?></h4>
								<div class="content text_note"></div>
							</div>
							<a class="button button-primary tbay-support" href="#" target="_blank"><?php esc_html_e( 'Get support', 'tbay-elementor' ); ?></a>
							<a class="button button-secondary tbay-visit-dashboard" href="<?php echo esc_url( get_admin_url() ); ?>"><?php esc_html_e( 'Dashboard', 'tbay-elementor' ); ?></a>
						</div>

						<div class="tbay-complete tbay-complete1">
							<h3 class=""><?php esc_html_e( 'Importing is successful!', 'tbay-elementor' ); ?></h3>
							<div class="content-message"></div> 
						</div>						

						<div class="tbay-complete tbay-complete2">
							<h3 class=""><?php esc_html_e( 'Switch homepage successfully!', 'tbay-elementor' ); ?></h3>
							<div class="content-message"></div> 
						</div>
						<br class="clear">
					</div>
