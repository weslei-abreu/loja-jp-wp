	...
	if ( ! class_exists( 'WooCommerce_simple_auction' ) ) {
		class WooCommerce_simple_auction {
			...
			public function init() {
				...
				add_action( 'woocommerce_product_duplicate', array( $this, 'woocommerce_duplicate_product' ) );
				...
				if ( is_admin() || current_user_can( 'manage_woocommerce' ) ) {
					...
					add_filter( 'woocommerce_simple_auctions_settings', array( $this, 'add_stripe_payment_options' ), 10 );
				...
				if ( ! is_admin() || defined( 'DOING_AJAX' ) ) {
					...
					add_filter( 'woocommerce_get_breadcrumb', array( $this, 'woocommerce_get_breadcrumb' ), 1 );
				...
				if ( is_object( $sitepress ) && function_exists( 'icl_object_id' ) && method_exists( $sitepress, 'get_default_language' ) ) {
				...
				add_action('woocommerce_simple_auctions_outbid',  array( $this, 'auctions_extend_time' ), 1);
				add_action('woocommerce_simple_auctions_proxy_outbid',  array( $this, 'auctions_extend_time' ), 1);

				add_action('woocommerce_simple_auction_won',  array( $this, 'stripe_automatic_charge' ), 1);


				add_filter( 'rank_math/pre_simple_page_id', array( $this, 'wpseo_get_page_id' ), 20 , 1  );
				add_filter( 'rank_math/paper/hash', array( $this, 'rank_math_paper_hash' ), 20 , 1  );
				add_filter( 'rank_math/frontend/canonical', array( $this, 'wpseo_canonical_and_og_url' ), 20 , 1 );
			...
			public function register_widgets() {
				...
				include_once 'classes/widgets/class-woocommerce-simple-auctions-widget-latest-bids.php';
				...
				register_widget( 'WC_SA_Widget_Latest_Bid_Auction' );
			}
			...
			public function admin_enqueue_script( $hook ) {
				...
				wp_enqueue_style( 'simple-auction-admin', $this->plugin_url . 'css/wsa-admin.css', array( 'woocommerce_admin_styles', 'jquery-ui-style' ) );
					...
					if ( 'product' == get_post_type() || $hook == 'woocommerce_page_auctions-activity' || $hook == 'woocommerce_page_auctions-winners' ) {
						...
						$params = array(
							...
							'datatable_language' => array(
							           ...
							           'print_table_history'   =>  sprintf( esc_html__('Auction history for "%s"', 'wc_simple_auctions' ) , $title ) ,
							        )
						);
						...
						if ( $product && $product->get_type() == 'auction' ){
							...
							wp_enqueue_script( 'buttons.print', plugin_dir_url( __FILE__ ) . 'js/DataTables/buttons.print.min.js', array( 'jquery', 'DataTables', 'DataTables-buttons' ), false );
						}
						...
						wp_register_script(
							...
							array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'timepicker-addon-auction', 'wc-admin-meta-boxes'),
							'1',
							true
						);
				...
				$custom_data = array(
					...
					'outbid_message'  => wc_get_template_html( 'notices/error.php',
						array(
							...
							'notices'  => array_filter( array( array( 'notice' => esc_html__( "You've been outbid!", 'wc_simple_auctions' ) ) ) ),
						)
					)
			...
			public function product_write_panel() {
				...
				$auction_dates_to   = ( $date = get_post_meta( $post->ID, '_auction_dates_to', true ) ) ? $date : '';
				 ...
				 echo '<p class="form-field auction_dates_fields">
						...
						<input type="text" class="short datetimepickerauction required" name="_auction_dates_from" id="_auction_dates_from" value="' . $auction_dates_from . '" placeholder="' . esc_html_x( 'From&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])"  />
						<input type="text" class="short datetimepickerauction required" name="_auction_dates_to" id="_auction_dates_to" value="' . $auction_dates_to . '" placeholder="' . esc_html_x( 'To&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])"  />
					</p>';
				woocommerce_wp_checkbox(
					array(
						'id'            => '_auction_extend_enable',
						'wrapper_class' => '',
						'label'         => esc_html__( 'Extend auction on bid?', 'wc_simple_auctions' ),
						'description'   => esc_html__( 'If a bid is placed during the final moments of an auction then the auction can be extended for a specified amount of time. This gives other bidders a chance to compete.', 'wc_simple_auctions' ),
						'desc_tip'      => 'true',
					)
				);
				woocommerce_wp_text_input(
					array(
						...
						'class'             => 'input_text',
						'size'              => '10',
						'label'             => __( 'Extend auctions in last * seconds.', 'wc_simple_auctions' ),
						'type'              => 'number',
						'custom_attributes' => array(
							'step' => 'any',
							'min'  => '0',
						...
						'desc_tip'          => 'true',
						'description'       => __( 'Extend auction in last N seconds. Enter 0 to extend always.', 'wc_simple_auctions' ),
					)
				);
				woocommerce_wp_text_input(
					array(
						...
						'class'             => 'input_text',
						'size'              => '10',
						'label'             => __( 'Extend auctions for * seconds.', 'wc_simple_auctions' ),
						'type'              => 'number',
						'custom_attributes' => array(
							'step' => 'any',
							'min'  => '0',
						...
						'desc_tip'          => 'true',
						'description'       => __( 'Extend auction for number of seconds.', 'wc_simple_auctions' ),
					)
				);

				if ( ( method_exists( $product, 'get_type' ) && $product->get_type() == 'auction' ) && $product->get_auction_closed() && ( ! $product->get_auction_payed() || get_option( 'simple_auctions_relist_paid', 'no' ) == 'yes' ) ) {
					echo '<p class="form-field relist_dates_fields"><a class="button relist" href="#" id="relistauction">' . esc_html__( 'Relist', 'wc_simple_auctions' ) . '</a></p>
                           <p class="form-field relist_auction_dates_fields"> <label for="_relist_auction_dates_from">' . esc_html__( 'Relist Auction Dates', 'wc_simple_auctions' ) . '</label>
							<input type="text" class="short datetimepickerauction " name="_relist_auction_dates_from" id="_relist_auction_dates_from" value="" placeholder="' . esc_html_x( 'From&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])" />
							<input type="text" class="short datetimepickerauction" name="_relist_auction_dates_to" id="_relist_auction_dates_to" value="" placeholder="' . esc_html_x( 'To&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])" />';
							woocommerce_wp_checkbox(
								array(
									'value'         => 'no',
									'id'            => '_auction_delete_log_on_relist',
									'wrapper_class' => 'relist_auction_dates_fields',
									'label'         => esc_html__( 'Delete logs on relist?', 'wc_simple_auctions' ),
									'description'   => esc_html__( "Delete all logs for this auction on relist. It can't be undone!", 'wc_simple_auctions' ),
									'desc_tip'      => 'true',
								)
							);
                  	echo '</p>';
				...
				if ( ! empty( $_POST['_auction_delete_log_on_relist'] ) ) {
					...
					if ( ! isset( $_POST['save_auction_data'] ) || ! wp_verify_nonce( sanitize_text_field( $_POST['save_auction_data'] ), 'save_auction_data_' . $post_id ) ) {
						...
						if ( 'yes' === sanitize_text_field( $_POST['_auction_delete_log_on_relist'] ) ) {
							self::del_auction_logs( $post_id );
						}
					}
				}
				delete_transient( 'wsa_term_counts_finished' );
				delete_transient( 'wsa_term_counts_future' );
			...
			public function relist_auction( $post_id ) {
				...
				if ( apply_filters( 'woocommerce_simple_auctions_relist_auction' , true , $product ) !== true ){
					return;
				}
				...
				if ( $product->get_auction_automatic_relist() == 'yes' && $product->is_finished() && $product->get_auction_relist_duration() ) {
					...
					$from_time = apply_filters('woocommerce_simple_auctions_relist_auction_from_time' , $from_time , $product );
					$to_time   = apply_filters('woocommerce_simple_auctions_relist_auction_to_time' , $to_time , $product );
					...
					if ( $product->get_auction_closed() == '3' && get_option( 'simple_auctions_relist_paid', 'no' ) === 'yes' )  {
			...
			public function woocommerce_simple_auctions_place_bid( $url = false ) {
				...
				$bid               = apply_filters( 'woocommerce_place_bid_bid', abs( round( floatval( str_replace( ',', '.', $_REQUEST['bid_value'] ) ), wc_get_price_decimals() ) ) );
				$was_place_bid     = false;
				$placed_bid        = array();
...
?>
					...
					<table class="auction-table widefat fixed" style="width:100%">
					<?php
					...
					?>
							...
							<thead>
								<tr>
									...
									<th  class="proxy"><?php esc_html_e( 'Auto', 'wc_simple_auctions' ); ?></th>
			...
			/**
			 *  Add watchlist link only to watchlist shortcode in loop
			 *
			 * @access public
			 *
			 */
			...
			public function auctions_page_template( $template ) {
				...
				if ( get_query_var( 'is_auction_archive', false ) ) {
					...
					$template_s = locate_template( WC()->template_path() . 'archive-product-auctions.php' );

					$template = ! empty( $template_s )? $template_s : $template;
			...
			public function remove_auctions_from_woocommerce_product_query( $q ) {
				...
				if ( ! is_admin() && $q->is_main_query() && $simple_auctions_dont_mix_cat === 'yes' && is_product_category() ) {
					$taxquery = $q->get( 'tax_query' );
					if ( ! is_array( $taxquery ) ) {
						$taxquery = array();
					}
					...
					$q->set( 'tax_query', $taxquery );
					$q->set( 'remove_auctions', 'true' );
				}
				if ( ! is_admin() && $q->is_main_query() && $simple_auctions_dont_mix_cat === 'yes' && is_product_tag() ) {
					$taxquery = $q->get( 'tax_query' );
					if ( ! is_array( $taxquery ) ) {
						$taxquery = array();
					}
			...
			public function pre_get_posts( $q ) {
				...
				if ( isset( $q->query_vars['remove_auctions'] ) && $q->query_vars['remove_auctions'] == 'true' ) {
					return;
				}
			...
			public function wpseo_get_page_id( $page_id ) {
				...
				if ( 'true' == get_query_var( 'auction_arhive', false ) ){
					return wc_get_page_id( 'auction' );
				}
					...
					if ( ! empty( $order_items ) && $order_items ) {
						...
						foreach ( $order_items as $item_id => $item ) {
							...
							if ( ! empty( $product_data ) && method_exists( $product_data, 'get_type' ) && $product_data->get_type() == 'auction' ) {
								...
								do_action( 'woocommerce_simple_auction_auction_payed', $item_meta['_product_id'][0]);
								if ( ! $order->get_meta('wsa_order_proccesed') ) {
									if ( ! $product_data->is_finished() ) {
										$original_product_id = intval( apply_filters( 'wpml_object_id', $product_data->get_id(), 'product', false, apply_filters( 'wpml_default_language', null ) ) );
				...
				if ( $order ) {
					...
					if ( $order_items ) {
						...
						foreach ( $order_items as $item_id => $item ) {
							...
							$order->update_meta_data( 'wsa_order_proccesed', time() );
							$order->save();
			 ...
			 *
			 */
			public function woocommerce_duplicate_product( $product ) {
			...
			public function simple_auctions_cron( $url = false ) {
				...
				if ( $_REQUEST['auction-cron'] == 'check' ) {
					...
					for ( $i = 0; $i < 3; $i++ ) {
						...
						$time = intval( microtime( true ) - $time );
			...
			*/
			function csv_export_activity() {
				...
				// Check for current user privileges (admins and shop managers)
				if( !current_user_can( 'manage_options' ) && !current_user_can( 'manage_woocommerce' ) ){ return false; }
			...
			*/
			public function auction_woocommerce_catalog_orderby( $data ) {
				...
				if ( ( is_product_category() && $simple_auctions_dont_mix_cat == 'yes'  ) && $is_auction_archive !== 'true' ) {
						return $data;
				}
				if ( ( is_product_tag() && $simple_auctions_dont_mix_tag == 'yes'  ) && $is_auction_archive !== 'true' ) {
						return $data;
				}
			...
			function add_stripe_payment_options( $data ) {
				$newdata = array();
				$disabled = true;
				if ( defined( 'WC_STRIPE_VERSION' ) ) {
					...
					$data[] = 	array(
						'title' => esc_html__( 'Stripe verification and auto payment for won auction options', 'wc_simple_auctions' ),
						'type' => 'title',
						'desc' => '',
						'id' => 'simple_auction_stripe_options'
						);

					$data[] =  array(
						'title' 			=> esc_html__( 'Do not allow bidding without Credit Card on file', 'wc_simple_auctions' ),
						'desc' 				=> esc_html__( '', 'wc_simple_auctions' ),
						'type' 				=> 'checkbox',
						'id'				=> 'simple_auctions_strip_nobid_without_card_enabled',
						'default' 			=> 'no',
						'disabled' => $disabled,
						);

					$data[] = array(
						'title' 			=> esc_html__( 'Automatically charge for won auction', 'wc_simple_auctions' ),
						'desc' 				=> esc_html__( '', 'wc_simple_auctions' ),
						'type' 				=> 'checkbox',
						'id'				=> 'simple_auctions_strip_automatic_carge_enabled',
						'default' 			=> 'no',
						'disabled' => $disabled,
					);
					$data[] = array( 'type' => 'sectionend', 'id' => 'simple_auction_stripe_options');
			...
			public function exclusion_finished_auctions_from_categories_widget( $terms, $taxonomy, $query_vars, $term_query ){
				...
				$o_term_counts = get_transient( 'wsa_term_counts_finished' );
				...
				foreach ($terms as $key => $value) {
					...
					if ( is_object( $value ) && $value->taxonomy == 'product_cat' ){
						...
						if ( ! isset( $term_counts[ $value->term_id ] ) ) {
							...
							$products = wp_cache_get( $key, 'wsa_cache' );
							if ( false === $products ) {
								$products = new WP_Query( $args );
								wp_cache_set( $key, $products, 'wsa_cache', 1);
							}
							$term_counts[ $value->term_id ] = $products->post_count;
						}

						if ( $term_counts[ $value->term_id ]) {
							$value->count = $value->count - $term_counts[ $value->term_id ];
						};
				...
				if ( $term_counts !== $o_term_counts ) {
					set_transient( 'wsa_term_counts_finished', $term_counts, DAY_IN_SECONDS * 7 );
				}
				return $terms;
			}
			...
			public function exclusion_future_auctions_from_categories_widget( $terms, $taxonomy, $query_vars, $term_query ){
				...
				$o_term_counts = get_transient( 'wsa_term_counts_future' );
				$term_counts   = false === $o_term_counts ? array() : $o_term_counts;
				foreach ($terms as $key => $value) {
					if ( is_object( $value ) && $value->taxonomy == 'product_cat' ){
						if ( ! isset( $term_counts[ $value->term_id ] ) ) {
				...
				// Update transient.
				if ( $term_counts !== $o_term_counts ) {
					set_transient( 'wsa_term_counts_future', $term_counts, DAY_IN_SECONDS * 7 );
				}
				return $terms;
			}
			...
			public function auctions_extend_time( $data ) {
				...
				if ( $product && $product->is_type( 'auction' ) ) {
					...
					$timeplus         = date( 'Y-m-d H:i:s', $time + $product->get_auction_extend_in_time() );
					...
					if ( $timeplus > $auctionendformat || $product->get_auction_extend_in_time() === 0 ) {
						...
						do_action( 'woocommerce_simple_auctions_after_time_extension', $product, $newdate );
					}
			...
			public function stripe_automatic_charge( $auction_id ){

				if ( ! defined( 'WC_STRIPE_VERSION' ) ||  get_option( 'simple_auctions_strip_automatic_carge_enabled', 'no') !== 'yes') {
					return;
				}
				$product = wc_get_product( $auction_id );
				$order = wc_create_order();

				$order->add_product( wc_get_product( $auction_id ), 1 ); //Add product to order
				$order->set_customer_id( $product->get_auction_current_bider() );
				$customer = new WC_Customer( $product->get_auction_current_bider() );
				$order->set_address( ! array_filter(  $customer->get_shipping() ) ? $customer->get_billing() : $customer->get_shipping(), 'shipping' ); //Add shipping address
				$order->set_address( $customer->get_billing(), 'billing' ); //Add billing address

				$stripe_customer = new WC_Stripe_Customer( $product->get_auction_current_bider() );

				$order->add_order_note( __('Atomatic charge by auction plugin', 'wc_simple_auctions') );

				$stripe_customer_payment_methods = ! empty( $stripe_customer->get_payment_methods('card') ) ? $stripe_customer->get_payment_methods('card')[0]->id : '';

				if ( ! empty ( $stripe_customer_payment_methods ) ){
					$order->update_meta_data( '_stripe_source_id', $stripe_customer_payment_methods );
					...
					if ( $order->get_billing_country() != '' ) {
						WC()->customer->set_billing_location( $order->get_billing_country(), $order->get_billing_state(), $order->get_billing_postcode(), $order->get_billing_city() );
						WC()->customer->set_shipping_location( $order->get_shipping_country(), $order->get_shipping_state(), $order->get_shipping_postcode(), $order->get_shipping_city() );
					} else {
					...
					if ( isset( $available_methods[0]['rates'] ) && ! empty( $available_methods[0]['rates'] ) ) {
						$cheapest_method = '';
					...
					if( ! empty( $cheapest_method ) ) {
						...
						$item->set_id( $cheapest_method->get_id() );
						$item->set_method_id( $cheapest_method->get_method_id() );
						$item->set_method_title( $cheapest_method->get_label() );

						$item->set_total($cheapest_method->get_cost());
						$order->add_item( $item );
					}
					$order->calculate_totals(true);
					$order->set_status("pending_payment"); //setting true included tax
					$order->save();
					//Try to charge stripe card
					$gateway      = new WC_Stripe_UPE_Payment_Gateway();
					$gateway->process_payment( $order->get_id(), true, true, false, true );
