...
<p class="wsawl-link">	
    <?php if ($product->is_user_watching()): ?>
    	<a href="#remove_from_watchlist" data-auction-id="<?php echo esc_attr( $product->get_id() ); ?>" class="remove-wsawl sa-watchlist-action"><?php esc_html_e('Remove from watchlist!', 'wc_simple_auctions') ?></a>
    <?php else : ?>
