...
class WC_Bid {
	...
	public function placebid ( $product_id, $bid ) {
		...
		if ( defined( 'WC_STRIPE_VERSION' ) && get_option( 'simple_auctions_strip_nobid_without_card_enabled', 'no') === 'yes') {
			$customer = new WC_Stripe_Customer( get_current_user_id() );
			if( ! $customer  || empty( $customer->get_payment_methods( 'card' ) ) ){
				wc_add_notice( sprintf( wp_kses_post( __('Sorry, you must set up your payment method to place a bid. <a href="%s" class="button">Set up &rarr;</a>', 'wc_simple_auctions') ),  wc_get_account_endpoint_url( 'payment-methods' ) ), 'error');
				return false;
			}
		}
				...
				if ($product_data->get_auction_proxy()) {
					if ($this->bid > (float)$product_data->get_auction_max_bid() OR  !$product_data->get_auction_max_bid()) {
						...
						if ($product_data->get_auction_reserved_price() && $product_data->is_reserve_met() === FALSE && apply_filters( 'woocommerce_simple_auctions_proxy_bid_to_reserve', true, $product_data ,$this->bid ) === true ) {
							...
							if ($product_data->get_auction_max_bid() &&  $product_data->get_auction_max_bid() > $product_data->get_auction_current_bid() ){
						...
						if($this->bid == (float)$product_data ->get_auction_max_bid()) {
							...
							$proxy_bid =  apply_filters( 'woocommerce_simple_auctions_proxy_bid_value' , $this->bid + (float)$product_data->get_auction_bid_increment() ,$product_data ,$this->bid);
