    ...
    <form method="POST" data-nonce="<?php echo esc_attr( wp_create_nonce( 'find-booked-day-blocks' ) ); ?>" id="wc-bookings-booking-form">
        <table class="form-table">
            ...
            <tr valign="top">
                <th scope="row">&nbsp;</th>
                <td>
                    <input type="submit" class="button-primary" value="<?php esc_attr_e( 'Add Booking', 'dokan' ); ?>" />
                    <input type="hidden" name="create_booking_2" value="<?php esc_attr_e( 'Add Booking', 'dokan' ); ?>" />
                    <input type="hidden" name="customer_id" value="<?php echo esc_attr( $customer_id ); ?>" />
                    <input type="hidden" name="bookable_product_id" value="<?php echo esc_attr( $bookable_product_id ); ?>" />
                    <input type="hidden" name="add-to-cart" class='wc-booking-product-id' value="<?php echo esc_attr( $bookable_product_id ); ?>" />
