...
/**
 * Vendor MangoPay Gateway Settings Template
 *
 * @since 3.5.0
 *
 * @package dokan
 ...
 * @var bool $is_seller_connected Checks if a seller is connected to MangoPay and ready to receive payment.
 * @var bool $is_payout_enabled Checks if a seller is eligible for payout.
 * @var object $mp_user MangoPay user object
 * @var array $payment_settings Payment settings
 * @var array $signup_fields Signup fields
 */
...
<?php if ( $is_seller_connected && ! empty( $mp_user ) ) : ?>
    <div class="dokan-alert dokan-alert-success dokan-text-middle">
        <?php esc_html_e( 'Your account is connected with MangoPay.', 'dokan' ); ?>
    </div>
...
<?php else : ?>
    <div class="dokan-alert dokan-alert-warning dokan-text-left" id="dokan-mangopay-account-notice">
        <?php esc_html_e( 'Your account is not connected with MangoPay. Update the informations below to complete signing up.', 'dokan' ); ?>
    </div>
<?php endif; ?>
