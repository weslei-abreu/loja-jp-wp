...
printf( __( 'Your order %1$s time has been updated by admin. %2$s', 'dokan' ), wp_strip_all_tags( $updated_delivery_type ), " \n\n" );
// translators: 1: Previous delivery date, 2: Previous delivery slot, 3: Previous delivery type, 4: Newline character.
printf( __( 'Previous delivery details : Date - %1$s, Slot - %2$s, Type - %3$s. %4$s', 'dokan' ), wp_strip_all_tags( $prev_delivery_date ), wp_strip_all_tags( $prev_delivery_slot ), wp_strip_all_tags( $prev_delivery_type ), " \n\n" );
// translators: 1: Updated delivery date, 2: Updated delivery slot, 3: Updated delivery type, 4: Newline character.
printf( __( 'Updated delivery details : Date - %1$s, Slot - %2$s, Type - %3$s. %4$s', 'dokan' ), wp_strip_all_tags( $updated_delivery_date ), wp_strip_all_tags( $updated_delivery_slot ), wp_strip_all_tags( $updated_delivery_type ), " \n\n" );
...
if ( 'store-pickup' === $updated_delivery_type ) :
    if ( 'store-pickup' === $prev_delivery_type ) :
        ...
        printf( __( 'Previous pickup location : %1$s. %2$s', 'dokan' ), wp_strip_all_tags( $prev_pickup_location ), " \n\n" );
    endif;

    // translators: 1: Updated pickup location, 2: Newline character.
    printf( __( 'Updated pickup location : %1$s. %2$s', 'dokan' ), wp_strip_all_tags( $updated_pickup_location ), " \n\n" );
endif;
