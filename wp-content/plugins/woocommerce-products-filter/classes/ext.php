...
abstract class WOOF_EXT {
    ...
    public static function is_ext_activated($full_path) {
        ...
        $ext_key = explode(DIRECTORY_SEPARATOR, $full_path);
        $ext_key = $ext_key[count($ext_key) - 1];
        ...
        if (self::is_ext_system($full_path)) {
            return $idx3;
        }
    ...
    //is ext is system one
    public static function is_ext_system($full_path) {
        $ext_info = WOOF_HELPER::parse_ext_data($full_path . DIRECTORY_SEPARATOR . 'info.dat');

        if (is_array($ext_info) && isset($ext_info['system'])) {
            return true;
        }
    ...
    //new from v.3.3.4
    public static function is_ext_a($key) {
        //by key is ext activated is_ext_a('front_builder')
        return self::is_ext_activated(WOOF_PATH . 'ext' . DIRECTORY_SEPARATOR . $key);
    }
