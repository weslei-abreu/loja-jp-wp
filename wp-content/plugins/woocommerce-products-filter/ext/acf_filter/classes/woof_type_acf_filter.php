...
abstract class WOOF_ACF_FILTER_TYPE {
    ...
    public function draw_meta_filter_structure() {
        ...
        <?php
        $show = 0;
        if (isset(woof()->settings[$this->meta_key]['show'])) {
            $show = woof()->settings[$this->meta_key]['show'];
        }
        ?>
            ...
            <a href="#" data-key="<?php echo esc_attr($this->meta_key) ?>" data-name="<?php echo esc_html($this->type_options['title']) ?>" class="woof-button js_woof_options js_woof_options_<?php echo esc_attr($this->meta_key) ?> help_tip" data-tip="<?php esc_html_e('additional options', 'woocommerce-products-filter') ?>"><span class="icon-cog-outline"></span></a>
