...
final class WOOF_EXT_BY_TEXT extends WOOF_EXT {
    ...
    public function woof_print_html_type_options() {
        ...
        woof()->render_html_e($this->get_ext_path() . 'views' . DIRECTORY_SEPARATOR . 'options.php', [
            'key' => $this->html_type,
            ...
            switch ($field) {
                ...
                case 'autocomplete':
                    if (isset($args['autocomplete'])) {
                        $data['autocomplete'] = intval($args['autocomplete']);
                    } elseif (isset(woof()->settings[$this->html_type]['autocomplete'])) {
                        $data['autocomplete'] = intval(woof()->settings[$this->html_type]['autocomplete']);
                    } else {
                        $data['autocomplete'] = 1;
                    }
                    break;
