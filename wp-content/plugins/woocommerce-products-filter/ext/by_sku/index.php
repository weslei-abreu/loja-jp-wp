...
final class WOOF_EXT_BY_SKU extends WOOF_EXT {
    ...
    public function wp_head() {
        //if (isset(woof()->settings['by_sku']['autocomplete']) AND woof()->settings['by_sku']['autocomplete']) {
            //for front builder this should be enabled (20-06-2023)
            ...
            if (!empty($request['woof_sku'])) {
                ...
                if (!empty($woof_sku_request)) {
                    foreach ($woof_sku_request as $k => $sku) {
                        ...
                        if ($logic == '=') {
							$condtion_string .= "postmeta.meta_value {$logic} '" . esc_sql($sku) . "'";
                        } else {
                            $condtion_string .= "postmeta.meta_value {$logic} '%" . esc_sql($sku) . "%'";
                        }
                    }
                ...
                if (!empty($product_variations)) {
                    ...
                    $products = $wpdb->get_results("
                        ...
                        FROM $wpdb->posts AS posts
                        WHERE posts.ID IN (" . esc_sql($product_variations_ids_string) . ") AND posts.post_parent > 0", ARRAY_N);
                    //+++
                    $product_ids = array();
                    if (!empty($products)) {
                    ...
                    $product_ids = esc_sql(implode(',', $result_ids));
    ...
    public function woof_sku_autocomplete() {
		...
		if (!wp_verify_nonce(WOOF_REQUEST::get('woof_sku_search_nonce'), 'sku_search_nonce')) {
            die('Stop!');
        }		
